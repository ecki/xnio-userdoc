...
The actual semantics for protocol selection and address binding depends on the actual XNIO provider used. For the NIO provider for example the IPv4/IPv6 semantic is influenced by the system properties `java.net.preferIPv4Stack` and `java.net.preferIPv6Addresses` (see for example http://docs.oracle.com/javase/7/docs/technotes/guides/net/index.html[Oracle Networking IPv6 User Guide]. It may also differ by VM vendor, version and runtime environment.
